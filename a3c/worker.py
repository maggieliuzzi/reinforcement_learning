...
class Worker:
  ...
  def update(self, steps, sess):
    ...
    for step in reversed(steps):
      reward = step.reward + self.discount_factor * reward  # reward should be return
      advantage = reward - self.get_value_prediction(step.state, sess)  # reward should be return
      # Accumulate updates
    ...
    global_step, pnet_loss, vnet_loss, _, _ = sess.run([
      self.global_step,
      self.policy_net.loss,  # just for plotting progress
      self.value_net.loss,   # just for plotting progress
      self.pnet_train_op,
      self.vnet_train_op,
    ], feed_dict)
