...
def ddpg(
  ...
  tf.set_random_seed(seed)  # tf.random.set_seed(seed)  # TF2
  np.random.seed(seed)
  ...
  print(env.observation_space, num_states, env.observation_space.high, env.observation_space.low)  # Box(3,) 3 [1. 1. 8.] [-1. -1. -8.]
  print(env.action_space, num_actions, env.action_space.high, action_max, env.action_space.low, action_min)  # Box(1,) 1 [2.] 2.0 [-2.] -2.0
  exit()
...
if __name__ == '__main__':
  ...
  parser.add_argument('--save_folder', type=str, default=None)  # 'ddpg_monitor'
